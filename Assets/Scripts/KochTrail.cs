...
public class KochTrail : KochGenerator
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < _initiatorPointAmount; i++)
        {
            ...
            trailObjectInstance.EmissionColor = _trailColor.Evaluate(i * (1.0f / _initiatorPointAmount)) * 255;
    ...
    void AudioBehaviour()
    {
        for (int i = 0; i < _initiatorPointAmount; i++)
        {
            ...
            //float widthLerp = Mathf.Lerp(_widthMinMax.x, _widthMinMax.y, _audioPeer._audioBandBuffer[_audioBand[i]] * 300);
            //_trail[i].Trail.widthMultiplier = widthLerp;

            //float timeLerp = Mathf.Lerp(_trailTimeMinMax.x, _trailTimeMinMax.y, _audioPeer._audioBandBuffer[_audioBand[i]] * 300);
